...
const MyOrders = () => {
  ...
  const cancelOrder = async (orderId) => {
    if (window.confirm("Are you sure you want to cancel this order?")) {
      try {
        const { data } = await axios.post(backendUrl + "/api/order/cancel", { orderId }, { withCredentials: true });
        if (data.success) {
                ...
                >
                  ...
                  <div className="flex flex-row sm:flex-col items-center sm:items-end gap-4 sm:gap-3 w-full sm:w-auto justify-between sm:justify-end mt-2 sm:mt-0 min-w-[140px]">
                    ...
                    {order.status === 'Order Placed' && (
                      <button onClick={() => cancelOrder(order._id)} className="w-full px-5 py-2.5 text-sm font-bold text-red-500 bg-red-50 rounded-xl hover:bg-red-100 transition-all duration-200 whitespace-nowrap flex items-center justify-center gap-2 active:scale-95 border border-red-100">
                        Cancel Order
                      </button>
                    )}
                  </div>
